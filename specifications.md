...
本システムは、複数のRSS/Atomフィードから当日（もしくは設定に応じた過去数日）の技術記事を収集し、以下の処理を自動化します。

- OpenAI APIを用いた記事本文の要約および技術用語抽出
- 収集済み記事・用語の重複排除（DynamoDB）
- Notionデータベースへの用語登録
- 記事一覧メールの自動配信（SES）
- CloudWatch Logsへの監査用ログ出力

Lambda上で定期実行するサーバーレス構成を想定しており、設定値やテンプレートはS3上のファイルとSecrets Managerから動的に読み込みます。

## 機能

- **フィード収集**: `feed_sources.json` に定義されたメディア／URLを基に、RSSとAtomの双方に対応して記事を取得します。
- **記事フィルタリング**: 設定されたタイムゾーンを基準に、収集対象日の記事のみを抽出します（デバッグ用に過去n日分を指定可能）。
- **要約・用語抽出**: OpenAI Chat Completions APIを利用して記事本文から要約と技術用語を生成します。
- **重複排除**: 記事URLおよび技術用語の正規化キーをDynamoDBに保存し、再収集・重複投稿を防ぎます。
- **Notion連携**: 生成した技術用語と説明文をNotionデータベースへ登録します。
- **メール配信**: S3管理のHTMLテンプレートを用いて記事一覧メールを構築し、SESで送信します。
- **監査ログ**: CloudWatch Logsへ収集結果（成功・失敗）を所定のフォーマットで記録します。

## 構成図

## ログ内容

CloudWatch Logs には以下の形式で情報を出力します。日付は収集対象日のタイムゾーンでフォーマットされます。
...
エラー発生時は例外内容を「エラーの理由」に記載し、そのままスタックに伝播させます（再試行時にCloudWatchログでトレース可能）。

## 実行フロー

1. Lambda 実行時に環境変数（`env.json`由来）を読み込み、S3 から `config.json`・`feed_sources.json`・`mail_body_template_file.html` を取得。
2. Secrets Manager から OpenAI/Notion/メールアドレス等の資格情報をロード。
3. 各フィードからRSS/Atomを取得し、対象日の記事のみ抽出。
4. 記事本文をOpenAIに送信して要約・用語抽出を実施。重複チェック後、DynamoDBへ書き込み。
5. Notionデータベースに抽出した技術用語を登録。
...
- **Notion API**: 技術用語データベースの更新
- **OpenAI API**: 記事要約・技術用語抽出
...
```json
{
  "aws": {
    ...
    "dynamodb": {
      "region": "ap-northeast-1",
      "articles_table_name": "RSSDistribute_article",
      "terms_table_name": "RSSDistribute_term",
      "article_attr": "source",
      "term_attr": "term",
